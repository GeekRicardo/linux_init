...
usermod -s /bin/zsh root && git config --global http.https://github.com.proxy ${PROXY} \
...
mkdir /var/run/sshd && echo 'root:ricardo' | chpasswd \
    && sed -i 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config \
    && sed 's@session\s*required\s*pam_loginuid.so@session optional pam_loginuid.so@g' -i /etc/pam.d/sshd \
    && echo "export VISIBLE=now" >> /etc/profile
export PATH=/root/.local/bin:${PATH}
wget -e "HTTPS_PROXY=${PROXY}" https://github.com/neovim/neovim/releases/download/v0.7.2/nvim-linux64.deb \
    ...
    && echo "alias vim=lvim"
wget -e "HTTPS_PROXY=${PROXY}" -O filebrowser.tar.gz https://github.com/filebrowser/filebrowser/releases/download/v2.22.3/linux-amd64-filebrowser.tar.gz \
...
wget -e "HTTPS_PROXY=${PROXY}" -O exa.zip https://github.com/ogham/exa/releases/download/v0.10.1/exa-linux-x86_64-v0.10.1.zip \
    && mkdir -p /usr/local/exa \
    && unzip exa.zip -d /usr/local/exa \
    && ln -s /usr/local/exa/bin/exa /usr/bin/exa \
    && echo "alias ls=exa" >> ~/.zshrc \
    && rm exa.zip
wget -O frp.tar.gz https://github.com/fatedier/frp/releases/download/v0.43.0/frp_0.43.0_linux_amd64.tar.gz \
    && tar zxvf frp.tar.gz -C /usr/local/frp \
    && rm frp.tar.gz
echo "alias gri=\"git rebase -i\"" >> ~/.zshrc \
